...
import { getPositionByWaitingTokenApi, getReservationTokenApi, getWaitingTokenApi } from "../api/TokenApi";
...
export default function ReserveWaiting() {
  ...
  useEffect(() => {
    getWaitingTokenApi(Number(concertId), Number(roundId))
    .then(setWaitingToken)
    .catch(err => {
      console.log("getWaitingTokenApi error!!", err)
    });
    }, []);
  ...
  useEffect(() => {
    ...
    // 5. 컴포넌트 언마운트 시 클린업
    return () => stopPolling();
  }, [waitingToken]); // 빈 배열을 두어 최초 마운트 시에만 인터벌이 1번 등록되도록 함
